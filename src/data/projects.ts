...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tagline: "AI-powered trip planning for Android",
    description:
      "A React Native + Expo Android app that takes destination, duration, style, and budget, then uses an LLM to generate detailed day-by-day itineraries with cost breakdowns.",
    longDescription:
      "A cross-platform mobile app that collapses trip planning into a single form. Users input destination, number of days, travel style (family, couple, or friends), and budget — and the app calls an LLM (Gemini / GPT) to return structured day-by-day itineraries with location recommendations, timings, ticket fares, hotel options, and per-day cost breakdowns. Built with React Native and Expo Router for smooth tab + stack navigation; user auth and data persistence run through Firebase Authentication and Firebase Realtime Database, with Google / Facebook sign-in. The most-starred repo in the profile, which is why it stays in the lineup despite being the oldest.",
    techStack: [
      "React Native",
      "Expo",
      "JavaScript",
    ...
    highlights: [
      "AI-generated personalized itineraries keyed on style and budget",
      "Day-by-day breakdown with locations, timings, ticket fares, and hotels",
      "Firebase Auth with Google / Facebook sign-in and Realtime Database",
      "Expo Router tab + stack navigation for smooth mobile UX",
      "Most-starred repo in the profile — real user adoption",
    ],
  ...
  },
  {
    ...
    tagline: "Reserve and release shared dev environments",
    description:
      "A Chrome extension backed by Vercel serverless functions and Supabase that lets dev and QA teams reserve, release, and track shared deployment environments with native notifications.",
    longDescription:
      "An internal-tool pattern made small and real. The Chrome extension popup + background service worker talks to Vercel serverless API routes, which persist state to Supabase (PostgreSQL). Developers can reserve or release an environment in one click, leave notes on reservations, and optionally notify QA users — who receive native Chrome OS notifications even without the popup open, plus an in-app bell with the last 20 notifications and read/unread status. Features include a real-time activity feed per environment category, stale-environment detection after 4+ hours, and search / filter by Backend / Frontend groups. Deployed on Vercel at envmanag.vercel.app.",
    techStack: [
      "JavaScript",
      "Chrome Extension",
      "Vercel Functions",
    ...
    highlights: [
      "Chrome extension + Vercel serverless + Supabase stack end-to-end",
      "Native Chrome OS notifications for QA users, even with popup closed",
      "Role-based flow (developer / QA) with notes and read/unread tracking",
      "Real-time activity feed and 4+ hour stale-environment detection",
      "Deployed live on Vercel — actually used, not just built",
    ],
    githubUrl: "https://github.com/kripa-sindhu-007/env_allocation",
    liveUrl: "https://envmanag.vercel.app",
    category: "Internal Tools",
    icon: "dns",
    codePreview: [
      "POST /api/reserve",
      "→ lock env, notify QA",
      "→ push chrome notification",
      "→ log activity",
    ],
  },
  {
    id: "browser-volume-control",
    title: "Browser Volume Control",
    tagline: "Per-tab volume without touching system audio",
    description:
      "A Chrome extension that controls the volume of the current browser tab independently of system audio — with presets, keyboard shortcuts, and a local-first, analytics-free design.",
    longDescription:
      "A tightly scoped browser extension that solves one annoying problem: one tab is too loud, another is too quiet, and muting the whole browser is too blunt. The TypeScript codebase uses the Web Audio API via a content-script GainNode to scale the current tab's output independently of system volume, with a popup UI for fine control and Quick, Mid, Loud, and Full presets. Tab-state detection surfaces inactive / live / muted / unavailable states, and the popup supports full keyboard control (M to mute, arrows for ±5% / ±10%, number keys for direct levels). The extension deliberately avoids broad host permissions at install time, has no analytics, no telemetry, and no account requirement.",
    techStack: [
      "TypeScript",
    ...
    highlights: [
      "Per-tab volume control via Web Audio GainNode — bypasses system volume",
      "Presets (Quiet / Mid / Loud / Full) with full keyboard control in the popup",
      "Tab-state detection: inactive / live / muted / unavailable",
      "No broad host permissions requested at install time",
      "Local-first: no analytics, no telemetry, no account required",
    ],
    githubUrl: "https://github.com/kripa-sindhu-007/browser_volume_control",
